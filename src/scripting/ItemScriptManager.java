...
public class ItemScriptManager extends AbstractScriptManager {
    ...
    public void start(MapleClient c, int npc, IItem item) {
        try {
            ...
            if (c.getPlayer().isGM()) {
                c.getPlayer().dropMessage(5, "開始道具腳本 NPC：" + npc + " 道具Id：" + item.getItemId());
            }
            ...
            System.err.println("執行道具腳本失敗 道具ID: (" + item.getItemId() + ")..NPCID: " + npc + ":" + e);
    ...
    public void action(MapleClient c, byte mode, byte type, int selection) {
        if (mode != -1) {
            ...
            try {
                ...
                System.err.println("執行NPC腳本出错 NPC ID : " + npcId + " 道具ID: " + itemId + " 錯誤信息: " + e);
                FilePrinter.printError("AbstractScriptManager.txt", e);
                dispose(c);
                notice(c, itemId);
            }
    ...
    private void notice(MapleClient c, int itemId) {
        c.getPlayer().dropMessage(1, "這個道具腳本是錯誤的 請回報GM " + itemId);
    }
}
