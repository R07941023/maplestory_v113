...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    public int gachapon(int type) {
        ...
        if (gitem.getSmegaType() > -1) {
            ...
            World.Broadcast.broadcastGachaponmega(MaplePacketCreator.getGachaponMega(c.getPlayer().getName() + " : x" + quantity + "恭喜" + c.getPlayer().getName() + "從楓葉轉蛋機獲得。", item, c.getChannel()));
        }
    ...
    public int gainGachaponItem(int id, int quantity, final String msg) {
        try {
            ...
            if (rareness == 1) {
                World.Broadcast.broadcastGachaponmega(MaplePacketCreator.getGachaponMega("[" + msg + "] " + c.getPlayer().getName() + " : 被他抽到了，大家恭喜他吧！", item, c.getChannel()));
            } else if (rareness == 2) {
                World.Broadcast.broadcastGachaponmega(MaplePacketCreator.getGachaponMega("[" + msg + "] " + c.getPlayer().getName() + " : 被他成功轉到了，大家恭喜他吧！", item, c.getChannel()));
            } else if (rareness > 2) {
                World.Broadcast.broadcastGachaponmega(MaplePacketCreator.getGachaponMega("[" + msg + "] " + c.getPlayer().getName() + " : 被他從楓葉轉蛋機轉到了，大家恭喜他吧！", item, c.getChannel()));
            }
    ...
    public void getMobs(int itemid) {
       ...
       for (int i = 0; i < mobs.size(); i++) {
           ...
           if (mi.getDropQuest(mobs.get(i)) > 0) {
            ...
            text += "#r#o" + mobs.get(i) +"##k "  + (quest > 0 && MapleQuest.getInstance(quest).getName().length() > 0 ? ("#b需要進行 " + MapleQuest.getInstance(quest).getName() + " 任務來取得#k") : "") + "\r\n";  
            
       }
       sendNext(text);
    }
   
     public void getMobs2(int itemid) {
       MapleMonsterInformationProvider mi = MapleMonsterInformationProvider.getInstance();
       final List<Integer> mobs = MapleMonsterInformationProvider.getInstance().getMobByItem(itemid);
       String text = "#d這些怪物會掉落您查詢的物品#k: \r\n\r\n";
    ...
    public void checkMobs(MapleCharacter chr) {
        ...
        String msg = "玩家 #b" + chr.getName() + "#k 該地圖掉落查詢:\r\n#r(若有任何掉落問題,请詢問GM)\r\n#d";
...
// 轉蛋
    ...
    public void addMaxHP(short hp) {//增加HP
    ...
    public void addMaxMP(short mp) {//增加MP
        Map<MapleStat, Integer> statup = new EnumMap<>(MapleStat.class);
...
/*    public String checkDrop(MapleCharacter chr, int mobId, boolean GM) {
        ...
        if (!ranks.isEmpty() && ranks.size() > 0) {
            ...
            name.append("【#r#o" + mobId + "##k】掉落物品查詢列表:#b" + "\r\n");
            for (int i = 0; i < ranks.size(); i++) {
                ...
                if (de.chance > 0 && (de.questid <= 0 || (de.questid > 0 && MapleQuest.getInstance(de.questid).getName().length() > 0))) {
                    ...
                    if (itemId == 0) { //meso
                        ...
                        if (GM == false) {
                            name.append("#k" + (num + 1) + ": #v" + itemId + "# " + namez + ((chr.isGM()) ? "#d  掉落機率：" + (Integer.valueOf(ch >= 999999 ? 1000000 : ch).doubleValue() / 10000.0) + "%\r\n" : "\r\n") + "#b(掉落條件:" + (de.questid > 0 && MapleQuest.getInstance(de.questid).getName().length() > 0 ? ("需要接受任務#r " + MapleQuest.getInstance(de.questid).getName() + " #b)\r\n") : "#r無#b)") + "\r\n");
                        } else {
                            name.append("#L" + itemId + "##k" + (num + 1) + ": #v" + itemId + "# " + namez + ((chr.isGM()) ? "#d  掉落機率：" + (Integer.valueOf(ch >= 999999 ? 1000000 : ch).doubleValue() / 10000.0) + "%(點擊更改)\r\n" : "\r\n") + "#b(掉落條件:" + (de.questid > 0 && MapleQuest.getInstance(de.questid).getName().length() > 0 ? ("需要接受任務#r " + MapleQuest.getInstance(de.questid).getName() + " #b)\r\n") : "#r無#b)") + "\r\n");
                        }
    ...
    public int getDropChance(int itemid, int dropper){
        ...
        try {
            ...
            throw new DatabaseException("連接錯誤", e);
        }
          return ret;
    }
...
//                    }
    ...
    public boolean hasSubwpn() {
        Item toUse1 = (Item) c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).getItem((short) -10);
        Item toUse2 = (Item) c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).getItem((short) -110);
    ...
    Equip item = (Equip)this.c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem((short)slot);
    Equip sel = item;
    Equip item2 = (Equip)this.c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem((short)slot2);
    Equip sel2 = item2;
    int itemid = this.c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem((short)slot).getItemId();
    int itemid2 = this.c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem((short)slot2).getItemId();
    ...
    String msg = "";
    if (isCash && isCash2) {
      msg = msg +"原本的點裝為#b#z"+ itemid + "##k\r\n";
      msg = msg +"轉移後的點裝為#b#z "+ itemid2 + "##k\r\n";
      sel2.setStr(sel.getStr());
      sel2.setDex(sel.getDex());
      sel2.setInt(sel.getInt());
      sel2.setLuk(sel.getLuk());
      sel2.setWatk(sel.getWatk());
      sel2.setMatk(sel.getMatk());
      ...
      MapleInventoryManipulator.addFromDrop(getClient(), (IItem)item2, false);
