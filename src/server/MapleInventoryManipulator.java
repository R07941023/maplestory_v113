...
public class MapleInventoryManipulator {
    ...
    public static void addRing(MapleCharacter chr, int itemId, int ringId, int sn) {
        CashItem csi = CashItemFactory.getInstance().getSimpleItem(sn);
        if (csi == null) {
            return;
        }
    ...
    public static byte addId(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period) {
         if (ServerConstants.LOG_TRACE_INVENTORY) {
            ...
            int nTrace = Math.min(e.getStackTrace().length, PiPiConfig.nStackTraceMax);
...
//            }
    ...
    public static void unequip(final MapleClient c, final short src, final short dst) {
        ...
        if (source.getItemId() == 1122017) {
          //  c.getPlayer().changeSkillLevel(SkillFactory.getSkill(4111006), (byte)20, (byte)20, -1);   
         //   c.getPlayer().changeKeybinding(50, (byte)1, 4111006); 
       //     c.getPlayer().changeChannel(2);            
            c.getPlayer().cancelFairySchedule(true);
        }

        if (source.getItemId() == 1112127) {
        //    c.getPlayer().changeSkillLevel(SkillFactory.getSkill(1221002), (byte)30, (byte)30, -1);   
        //    c.getPlayer().changeKeybinding(40, (byte)1, 1221002); 
       //     c.getPlayer().changeChannel(2);            
            c.getPlayer().cancelFairySchedule(true);
        }
    ...
    public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, short quantity, final boolean npcInduced) {
        ...
        if (quantity < source.getQuantity() && !GameConstants.isRechargable(source.getItemId())) {
            ...
            if (ii.isDropRestricted(source.getItemId()) || ii.isAccountShared(source.getItemId())) {
                ...
                c.getPlayer().getMap().spawnItemDrop(c.getPlayer(), c.getPlayer(), source, dropPos, true, true);
                  FileoutputUtil.logToFile("logs/Data/丟棄道具.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 帳號 " + c.getAccountName() + " 帳號ID " + c.getAccID() + " 角色名 " + c.getPlayer().getName() + " 角色ID " + c.getPlayer().getId() + " 類型 " + type + " 在地圖: " + c.getPlayer().getMap().getMapName() + "頻道：" + c.getChannel() +" 道具唯一ID: " + source.getEquipOnlyId()  + " 物品 " + ii.getName(source.getItemId()) + " (" + source.getItemId() + ") x" + quantity);              
            }
        }
        return true;
    }
