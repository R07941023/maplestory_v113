                    ...
                    if (c.getPlayer() != null) {
                        ...
                        try {
                            ...
                            if (ret) {
                                ...
                                FileoutputUtil.logToFile("logs/Data/GM指令.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 帳號: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用GM指令:" + line);
                                // 訊息處理
                                ShowMsg(c, line, type);
                            } else {
                                c.getPlayer().dropMessage("指令錯誤，用法： " + co.getMessage());
