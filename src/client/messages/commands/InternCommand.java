...
public class InternCommand {
    ...
    public static class 生死活動 extends CommandExecute {

        @Override
        public boolean execute(MapleClient c, String[] splitted) {
            MapleCharacter chr = c.getPlayer();
            if (splitted.length < 2) {
                c.getPlayer().dropMessage(5, "[用法] !生死活動 <1幹死右邊，2幹死左邊>");
                return false;
            }

            if (chr.getMapId() != 109020001) {
                c.getPlayer().dropMessage(5, "只有地圖ID為109020001的地圖才能使用。");
                return true;
            }
            ...
            for (MapleCharacter chra : chrlist) {
                if (生死活動.isCorrectAnswer(chra, answer)) {
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append("!").append(getClass().getSimpleName().toLowerCase()).append(" <1幹死右邊，2幹死左邊>").toString();
        }
