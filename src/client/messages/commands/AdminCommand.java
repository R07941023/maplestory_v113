...
//                    c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.spawnNPC(npc, true));
    ...
    public static class GiveItem extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            if (splitted.length < 4) {
                c.getPlayer().dropMessage("!GiveItem 角色名稱 物品ID 給予數量");
                return false;
            }
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append(ServerConstants.PlayerGMRank.超級管理員.getCommandPrefix()).append("GiveItem 角色名稱 物品ID 給予數量 - 單一玩家發送物品").toString();
        }
   }
  
    public static class GiveallItem extends CommandExecute {

        @Override
        public boolean execute(MapleClient c, String[] splitted) {
            if (splitted.length < 3) {
                c.getPlayer().dropMessage(5, "!GiveallItem 物品 數量");
                return false;
            }
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append(ServerConstants.PlayerGMRank.超級管理員.getCommandPrefix()).append("GiveallItem 物品 數量 - 全服發送物品").toString();
       } 
    }
    ...
    public static class ReloadGashapon extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String splitted[]) {
            ...
            c.getPlayer().dropMessage(6, "轉蛋機已重載完成");            
            return true;
        }

        @Override
        public String getMessage() {
            return new StringBuilder().append(ServerConstants.PlayerGMRank.超級管理員.getCommandPrefix()).append("reloadGashapon - 重新載入轉蛋機").toString();
        }
    } 
