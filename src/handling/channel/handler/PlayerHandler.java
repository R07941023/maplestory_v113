...
//        }
        ...
        if (GameConstants.isFishingMap(chr.getMapId())) {
            ...
            for (IItem item : c.getPlayer().getInventory(MapleInventoryType.CASH).list()) {
                if (itemId == 3010001 && item.getItemId() == 5340000) {
                    haz = true;
                    gg = true;
                } else if (itemId / 10000 == 301 && item.getItemId() == 5340001) {
                    haz = false;
                    gg = true;
        ...
        int originx = chr.getPosition().x;
        int originy = chr.getPosition().y;
        
     
        final List<Pair<MapleBuffStat, Integer>> stat = Collections.singletonList(new Pair<>(MapleBuffStat.MONSTER_RIDING,0));
        chr.getClient().getSession().write(MaplePacketCreator.giveMount(GameConstants.isFlyChair(itemId), 0,stat));
        //chr.getClient().getSession().write(MaplePacketCreator.giveMount(parseMountInfo_Pure(chr, itemId), itemId,stat));
        //chr.getMap().broadcastMessage(chr, MaplePacketCreator.showMonsterRiding(chr.getId(), stat, parseMountInfo(chr, itemId), itemId), false);
        chr.getMap().broadcastMessage(chr, MaplePacketCreator.showChair(chr.getId(), itemId), false);
...
//	slea.readShort(); // Original Y pos
    ...
    public static final void AranCombo(MapleClient c, MapleCharacter chr, int toAdd) {
        if ((chr != null) && (chr.getJob() >= 2000) && (chr.getJob() <= 2112)) {
            ...
            if ((combo > 0) && (curr - chr.getLastCombo() > 4000L)) {
                combo = 0;
            }
