...
public class InventoryHandler {
   ...
   final MapleInventory pInv = c.getPlayer().getInventory(pInvType); //取得玩家該類型的道具欄
   final ArrayList<IItem> itemList = new ArrayList<IItem>();
   if(pInvType != MapleInventoryType.CASH){//非特殊欄位整理
      for(IItem item : pInv.list()){
      ...
      IItem item;
      short idx = 1;
      byte maxSlot = pInv.getSlotLimit();
      for(short i=1; i<maxSlot; i++){
         item = pInv.getItem(i);
         if(item != null){
            itemId = item.getItemId();
            if(GameConstants.isPet(itemId)){
               MapleInventoryManipulator.move(c, pInvType, i, idx);
               idx++;
            }
         }
      }
      
      for(short i=idx; i<maxSlot; i++){
         item = pInv.getItem(i);
         if(item != null){
            itemList.add(item.copy()); // clone all items
         }
      }
  }
   //２ 按照道具編號小 排到 大
   itemList.sort((itemA, itemB) -> itemA.getItemId() - itemB.getItemId());


   //３ 清除該背包的道具
   for(IItem itemStats : itemList) {
      MapleInventoryManipulator.removeById(c, pInvType, itemStats.getItemId(), itemStats.getQuantity(), true, false);
   }      
    ...
    public static final void PlayerPickup(final LittleEndianAccessor slea, MapleClient c, final MapleCharacter chr) {
        ...
        try {
            ...
            if (ServerConstants.log_pickup && ((mapitem.getItem() != null && mapitem.getItem().getEquipOnlyId() > 0) || (GameConstants.isAllScroll(mapitem.getItemId()) || GameConstants.isOpScroll(mapitem.getItemId())))) {
                FileoutputUtil.logToFile("logs/data/人物撿道具.txt", FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 帳號 " + c.getAccountName() + " 帳號ID " + c.getAccID() + "角色名字:" + c.getPlayer().getName() + " 在地圖: " + c.getPlayer().getMap().getMapName() + "頻道：" + c.getChannel() + " 從地板撿取了: " + MapleItemInformationProvider.getInstance().getName(mapitem.getItemId()) + "(" + mapitem.getItemId() + ") 數量:" + mapitem.getItem().getQuantity() + " 道具唯一ID: " + mapitem.getItem().getEquipOnlyId() + " \r\n");
            }
